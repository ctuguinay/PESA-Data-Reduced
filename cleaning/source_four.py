from helper import replace_emojis, general_text_cleaning, traverse_new, utc_seconds_to_datetime, remove_duplicates
import csv
import os
...
def traverse_original(uncleaned_path):
    ...
    with open(uncleaned_path, 'r', encoding = "utf-8") as unlabeled_file:
        ...
        for index, row in enumerate(reader):
            ...
            row[0] = mod_text
            if row[2] == "comment":
                utc_seconds = float(row[5])
                row[5] = utc_seconds_to_datetime(utc_seconds = utc_seconds)
            rows.append(row)
