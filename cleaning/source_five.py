from helper import replace_emojis, general_text_cleaning, traverse_new, remove_duplicates, remove_duplicate_text
import csv
import os
...
def traverse_original(uncleaned_path):
    ...
    with open(uncleaned_path, 'r', encoding = "utf-8") as unlabeled_file:
        ...
        for index, row in enumerate(reader):
            ...
            if len(mod_text) > 2:
                if not(mod_text[0:3] == "rt " or mod_text[0:2] == "rt"):
                    row[0] = mod_text
                    rows.append(row)
            elif len(mod_text) > 1:
                if not(mod_text[0:2] == "rt"):
                    row[0] = mod_text
                    rows.append(row)
...
if __name__ == "__main__":
    ...
    # Remove duplicate text values rows.
